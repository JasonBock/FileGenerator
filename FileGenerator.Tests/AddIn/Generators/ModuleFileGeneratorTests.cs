...
{
	[TestClass]
	public sealed class ModuleFileGeneratorTests : CoreTests
	{
		private void Create(string languageName, string projectExtension, string fileExtension)
		{
			var item = AssemblyTests.GetModule("FileGenerator.Tests.Target");
			...
			using(var @event = new ManualResetEvent(false))
			{
				...
				Assert.IsTrue(File.Exists(Path.Combine(outputPath, "FileGenerator.Tests.Target.dll" + projectExtension)));
		...
		{
			this.Create(LanguageInformation.CSharp,
		...
		{
			this.Create(LanguageInformation.VB,
		...
		{
			this.Create(LanguageInformation.IL,
		...
		private void CreateWithNoProjectFile(string languageName, string projectExtension, string fileExtension)
		{
			...
			using(var @event = new ManualResetEvent(false))
			{
				...
				Assert.IsFalse(File.Exists(Path.Combine(outputPath, "FileGenerator.Tests.Target.dll" + projectExtension)));
				Assert.IsTrue(File.Exists(Path.Combine(outputPath, "_Module_" + fileExtension)));
